...
Live audio capture/playback (and the playing `<video>`) make macOS hold `PreventUserIdleSystemSleep` (via `coreaudiod`) and a Chromium "Video Wake Lock" (`NoDisplaySleepAssertion`), which keep the machine awake while Carabiner streams. The app can't suppress those assertions directly — the only way to release them is to stop capturing. This is macOS-only: the toggle is hidden on Windows/Linux (`DisplaySection.js`) and the watcher only starts when `isMacOS`. When `display.allowSleep` is enabled (default; toggle in the Display tab → `set-allow-sleep`), `main.js` registers a `powerMonitor` watcher: it pauses capture on `lock-screen`, and on idle once `getSystemIdleTime()` reaches the user's own macOS **display-sleep / screen-saver delay** (read live via `pmset -g` `displaysleep` and `defaults -currentHost read com.apple.screensaver idleTime`, re-read every 5 min so it tracks AC/battery changes). The idle threshold is deliberately not a fixed timeout — it matches when the screen saver / display sleep would have fired, so passively watching the stream does not pause it; if both are "Never" the idle path is skipped and only screen lock pauses. Idle-triggered pauses auto-resume on activity; lock-triggered pauses wait for `unlock-screen` (tracked via `suspendedByIdle`). Pause/resume reuse the display window's existing `stopVideoStream()`/`renderDisplay()` via the dedicated `auto-suspend`/`auto-resume` IPC messages (kept separate from `window-hide`/`window-show`). The watcher is started/stopped live when the toggle changes and torn down on `before-quit`.
