...
| `menu.js` | macOS app menu, system tray (Win/macOS), right-click context menu. **Display Windows** submenu (per capture device: **Enabled** + **Visible** checkboxes) in the menu bar, macOS View menu, and the right-click context menu; **Linked Device** submenu (relink the active window's control); a disabled **"Active Window: …"** indicator; window-specific items are disabled when no window is active; per-window titles in the macOS Window menu. `setWindowActions()` wires the submenu to main. Also the Automation scripts submenu and "Check for Updates…" |
